...
images.get('/', async (req,res):Promise<void> => {
	try {
		...
		filelist.forEach((filename:string) => responseBody += filename + '<br>');
		responseBody += "<br>To view a file, access api/images/filename. Include the file extension.<br>";
		responseBody += "To resize a file, access api/images/filename?height=newheight&width=newwidth. Again, include the file extension.<br>";
...
images.get('/:id', async (req,res):Promise<void> => {
	...
	if (resize_flag) {
		...
		if (isNaN(width) || isNaN(height)) {
			res.status(422).send("Height and width parameters must be integers");
			return;
		}
	}
	...
	try {
		...
		//Finally, do the hard part of sending a resized file if that is what is requested
			...
			const resizedFileName = req.params.id.slice(0,-4) + 'size' 
									+ width + 'x' + height + req.params.id.slice(-4);						
			if (cachelist.includes(resizedFileName)) {
				res.status(200).sendFile(resizedFileName, {root:cachePath});
			} else {
				try{
					await img_proc.resize(path.join(imagesPath, req.params.id),
								path.join(cachePath, resizedFileName),
								width, height);
					res.status(200).sendFile(resizedFileName, {root:cachePath});
				} catch (err) {
					console.log(err);
					res.status(500).send("File resizing encountered an error.");
					return;
				}				
			}
		}			
